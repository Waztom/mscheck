...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if "Analyzing" in result:
            result = result.replace("Analyzing", f"{Colors.BLUE}Analyzing{Colors.RESET}")
        
        if "Processing sample" in result:
            result = result.replace("Processing sample", f"{Colors.CYAN}Processing sample{Colors.RESET}")
            ...
            if len(parts) > 1:
                try:
                    ...
                    if value > 50.0:
                        ...
                        result = f"{parts[0]}EIC Area: {Colors.GREEN}{value}{Colors.RESET}"
                    else:
                        result = f"{parts[0]}EIC Area: {Colors.YELLOW}{value}{Colors.RESET}"
                except (ValueError, IndexError):
                    pass
        
        if "Successfully" in result:
            result = result.replace("Successfully", f"{Colors.BRIGHT_GREEN}Successfully{Colors.RESET}")
        ...
        for compound_type in ["product", "reactant", "internal-std", "intermediate"]:
            if compound_type in result:
                ...
                result = result.replace(compound_type, f"{color}{compound_type}{Colors.RESET}")
