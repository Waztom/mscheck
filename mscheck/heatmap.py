...
class MSHeatmapGenerator:
    ...
    def __init__(self, data: pd.DataFrame, output_dir: str = None):
        ...
        # Log initialization
        self.logger.info(f"Initialized MSHeatmapGenerator with output directory: {output_dir}")
        ...
        if self.output_dir:
            ...
            self.logger.debug(f"Created output directories: {self.heatmap_dir}, {self.comparison_dir}")
    ...
    def generate_heatmap(
        ...
        has_plate_layout = plate_column in self.data.columns and well_column in self.data.columns
    ...
    def generate_plate_comparison(
        ...
        try:
            ...
            bars = sns.barplot(x='plate', y='mean', data=stats_df, ax=ax, palette=colormap)
        ...
        if product_column in self.data.columns and reactant_column in self.data.columns:
            try:
                self.data[ratio_column_name] = pd.to_numeric(
                    self.data[product_column], 
                    errors="coerce"
                ) / pd.to_numeric(
                    self.data[reactant_column], 
                    errors="coerce"
                )
                print(f"Calculated conversion ratio as {product_column} / {reactant_column}")
                return True
            except Exception as e:
            ...
            print(f"Cannot calculate conversion ratio - missing columns: {', '.join(missing)}")
            return False
        ...
        for column, title, cmap in metrics:
            if column in self.data.columns:
                try:
                    ...
                    error_msg = f"Error generating visualizations for {column}: {str(e)}"
                    self.errors.append(error_msg)
                    print(error_msg)
                    traceback.print_exc()
            else:
